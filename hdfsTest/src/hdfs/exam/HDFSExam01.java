...
/*
 * hadoop에 hdfs를 api로 제어
 *  - hadoop hdfs에 api를 이용해서 파일 생성
 *  - 사용자가 지정하는 경로에 파일 생성 
 *  
 */
public class HDFSExam01 {
	public static void main(String[] args) {
	 //1.hdfs를 제어하기 위해서 설정 파일을 읽어서 사용해야 하므로 
		...
		//2.HDFS를 접근하기 위해서 제공되는 객체를 생성 - HDFS객체
		FileSystem hdfs = null;
		//3.HDFS에 Output하기 위한 스트림객체
		FSDataOutputStream hdfsout = null;
		//FileWriter fw = null;처럼 정의하는 것과 동일
	
		try {
			hdfs = FileSystem.get(conf);
			//일반적인 경로가 아니기 때문에 path 로 경로를 설정해준다.
